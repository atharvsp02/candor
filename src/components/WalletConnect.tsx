...
export function WalletConnect({ status, onConnect, onDisconnect }: Props) {
  ...
  return (
    <div className="wallet">
      <button className="btn btn-blue" onClick={onConnect} disabled={status.kind === 'connecting'}>
        {status.kind === 'connecting' ? 'Connecting…' : 'Connect wallet'}
      </button>
      {status.kind === 'error' && <p className="wallet-error">{status.message}</p>}
    </div>
  );
}
